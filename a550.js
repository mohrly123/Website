...
function abstand550(){
    ...
    if(eingabeA >= minabstand && eingabeA <= maxabstand){
        ...
        if(eingabeA < sollabstand){
            ...
            document.getElementById("inputIstAbstand").focus();
...
function höhe550(){
    ...
    if (eingabeH >= minhöhe && eingabeH <= maxhöhe){
        if (eingabeH < sollhöhe){
            ...
            document.getElementById("inputIstHöhe").focus();
            setTimeout(textVerschwinden, 6000);
        }
        else if(eingabeH > sollhöhe){
            let ergebnis = eingabeH - sollhöhe;
            ergebnis = Number(ergebnis);
            document.getElementById("resultHöhe550").textContent = `Hebung = ${ergebnis} mm`;
            document.getElementById("inputIstHöhe").value = "";
            document.getElementById("inputIstHöhe").focus();
            setTimeout(textVerschwinden, 6000);
        }
        else{
            document.getElementById("resultHöhe550").textContent = `Das Gleis hat 0 mm Hebung`;
            document.getElementById("inputIstHöhe").value = "";
            document.getElementById("inputIstHöhe").focus();
            setTimeout(textVerschwinden, 6000);
        }
